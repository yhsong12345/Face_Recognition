...
from utils.loss import ArcFace, SubcenterArcFace, ElasticArcFace  #ElasticCosFace
from models.Resnet import LResNet
from models.common import SEBasicBlock, SEBottleneck, ISEBottleneck
...
class Resnet(L.LightningModule):
    def __init__(self, model_name, pretrained=False, num_classes=10, loss=None):
        ...
        if model_name == 'resnet50':
            ...
            model = LResNet(ISEBottleneck, [3, 4, 6, 3], nn.PReLU(), num_classes=num_classes)
        elif model_name == 'SE_LResnet101_IR':
            model = LResNet(ISEBottleneck, [3, 4, 23, 3], nn.PReLU(), num_classes=num_classes)
        ...
        if loss == 'arcface':
            ...
            self.head = ElasticArcFace(embed_size=512, num_classes=num_classes,
                                       s=64.0, m=0.5, std=0.0125)
        else:
            pass
...
class Face_Recognition(L.LightningModule):
    def __init__(self, model_name, lr, pretrained=False, num_classes=10, loss='arcface'):
        super().__init__()
        self.lr = lr

        self.model = Resnet(model_name, pretrained=pretrained, num_classes=num_classes, loss=loss)
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.SGD(params=[{"params": self.model.parameters()}], lr=self.lr,
                                    momentum=0.9, weight_decay=5e-4)

        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 56, 64], gamma=0.1)
